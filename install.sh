...
if [ "$OS" = redhat  ] ; then
	...
	if [ `echo $REL | grep -c "pve-manager/4" ` -gt 0 ] ; then
        	apt-get install -y kpartx gcc openssl unzip make e2fsprogs gperf genisoimage flex bison pkg-config libpcre3-dev libreadline-dev libxml2-dev ocaml libselinux1-dev libsepol1-dev libfuse-dev libyajl-dev >> $LOG 2>&1		
	else
        	apt-get install -y kpartx gcc openssl unzip make e2fsprogs gperf genisoimage flex bison pkg-config libpcre3-dev libreadline-dev libxml2-dev ocaml libselinux1-dev libsepol1-dev libyajl-dev>> $LOG 2>&1
...
# Get our installer
wget -O /usr/local/virtualizor/install.php https://raw.githubusercontent.com/systemroot/kvm-virt-ovh/master/install.php >> $LOG 2>&1
